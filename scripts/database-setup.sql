-- Database setup for reqquli requirements management system
-- PostgreSQL 15+ with UUID and array support
...
-- System Requirements (no longer has traceFrom - uses junction table)
...
CREATE INDEX idx_sr_status_created ON system_requirements(status, created_at);
CREATE INDEX idx_sr_deleted_status ON system_requirements(deleted_at, status);
CREATE INDEX idx_sr_active ON system_requirements(deleted_at) WHERE deleted_at IS NULL;
CREATE INDEX idx_token_blacklist_expires ON token_blacklist(expires_at);

CREATE INDEX idx_user_requirements_status ON user_requirements(status) WHERE deleted_at IS NULL;
CREATE INDEX idx_user_requirements_created_by ON user_requirements(created_by);
...
-- Create sequence for test case numbering
CREATE SEQUENCE IF NOT EXISTS test_case_seq START WITH 1;

-- Create sequence for test run numbering
CREATE SEQUENCE IF NOT EXISTS test_run_seq START WITH 1;
...
-- Test Steps
CREATE TABLE testing.test_steps (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    test_case_id VARCHAR(20) REFERENCES testing.test_cases(id) ON DELETE CASCADE,
...
CREATE TABLE testing.test_runs (
    ...
    description TEXT,
    status VARCHAR(20) NOT NULL DEFAULT 'not_started' CHECK (status IN ('not_started', 'in_progress', 'complete', 'approved')),
    overall_result VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (overall_result IN ('pass', 'fail', 'pending')),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    created_by UUID REFERENCES users(id),
    approved_at TIMESTAMP WITH TIME ZONE,
    approved_by UUID REFERENCES users(id)
...
-- Test Run Cases (links test cases to runs)
CREATE TABLE testing.test_run_cases (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    test_run_id VARCHAR(20) REFERENCES testing.test_runs(id) ON DELETE CASCADE,
...
-- Test Step Results
CREATE TABLE testing.test_step_results (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    test_run_case_id UUID REFERENCES testing.test_run_cases(id) ON DELETE CASCADE,
    step_number INTEGER NOT NULL,
    expected_result TEXT NOT NULL,
    actual_result TEXT,
    status VARCHAR(20) NOT NULL DEFAULT 'not_executed' CHECK (status IN ('pass', 'fail', 'not_executed')),
    evidence_file_id UUID,
    executed_at TIMESTAMP WITH TIME ZONE,
    executed_by UUID REFERENCES users(id),
    UNIQUE(test_run_case_id, step_number)
);

-- Evidence Files
CREATE TABLE testing.evidence_files (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
...
-- Note: SR to TC links are now handled via the main traces table
-- No separate requirement_test_links table needed

-- Indexes for testing tables
CREATE INDEX idx_test_cases_status ON testing.test_cases(status) WHERE deleted_at IS NULL;
CREATE INDEX idx_test_runs_status ON testing.test_runs(status);
CREATE INDEX idx_test_runs_created_by ON testing.test_runs(created_by);
CREATE INDEX idx_test_run_cases_test_run ON testing.test_run_cases(test_run_id);
CREATE INDEX idx_test_run_cases_test_case ON testing.test_run_cases(test_case_id);
CREATE INDEX idx_test_step_results_run_case ON testing.test_step_results(test_run_case_id);
-- Indexes for test results
CREATE INDEX idx_test_results_test_case ON testing.test_results(test_case_id);
CREATE INDEX idx_test_results_test_run ON testing.test_results(test_run_id);
CREATE INDEX idx_test_results_result ON testing.test_results(result);
-- SR to TC trace indexes are handled by the main traces table indexes

-- =============================================
-- AUDIT EVENTS SCHEMA
-- =============================================
-- Comprehensive audit events tracking for all domain events
...
BEGIN
    ...
    IF TG_OP = 'INSERT' THEN
        ...
        ELSIF TG_TABLE_NAME = 'email_verification_tokens' THEN
            ...
            v_aggregate_id := NEW.user_id::VARCHAR;
    ...
    IF v_event_type IS NOT NULL THEN
        INSERT INTO audit_events (
            ...
            'Authentication', v_event_type, 'User', v_aggregate_id,
...
BEGIN
    ...
    -- Set aggregate_id based on table type
    IF TG_TABLE_NAME IN ('test_cases', 'test_runs', 'evidence_files') THEN
        v_aggregate_id := NEW.id::VARCHAR;
    ELSIF TG_TABLE_NAME = 'test_run_cases' THEN
        v_aggregate_id := NEW.test_run_id || '-' || NEW.test_case_id;
    ELSIF TG_TABLE_NAME = 'test_step_results' THEN
        v_aggregate_id := NEW.test_run_case_id::VARCHAR;
    END IF;
    ...
    IF v_event_name IS NOT NULL THEN
        INSERT INTO audit_events (
            ...
            v_event_type, v_event_name, v_aggregate_type, v_aggregate_id,
            v_user_id, v_user_email, v_user_name, v_event_data, NOW()
        );
    END IF;
